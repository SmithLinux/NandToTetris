        ...
        for line in f:
            ...
            if not isSpace(line) and not isEmptyLine(line):
                if not isLabel(line):
                    ...
                    INS_ADDRESS[line] = count

                lists.append(line)

            #if not isLabel(line) and not isSpace(line) and not isEmptyLine(line):
...
def symbolToNumber(symbol):
    global var_start
    i = '('+symbol+')'
    if symbol.isdigit(): #number
        return symbol
    elif i in INS_ADDRESS: #label
        return INS_ADDRESS[i]
    elif symbol not in SYMBOL_TABLE: #variable
        SYMBOL_TABLE[str(symbol)] = str(var_start)
...
def addLIns(line):
    ...
    if item not in SYMBOL_TABLE:
        ...
        address = INS_ADDRESS[line]
        SYMBOL_TABLE[str(item)] = str(address)
...
# 2. index of is not working ,because (loop) is the adress of the next instruction, The (loop) doesn't count
